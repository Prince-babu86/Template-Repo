...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res = await AuthApi.register({ fullname, email, password });
      alert(res.data);
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-black px-4">
      ...
      <div className="w-full max-w-md bg-[#0a0a0a] border border-gray-800 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.8)] p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Signup Button */}
          <button className="w-full py-3 rounded-xl font-medium text-black bg-white hover:bg-gray-200 transition">
            Sign Up
          </button>
        </form>
